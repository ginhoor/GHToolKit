...
public class Fuse {
    ...
    fileprivate lazy var searchQueue: DispatchQueue = { [unowned self] in
        return DispatchQueue(label: "fuse.search.queue", attributes: .concurrent)
    }()
    
    /// Creates a new instance of `Fuse`
    ///
    /// - Parameters:
    ///   - location: 希望在文本中找到的模式的大概位置。Approximately where in the text is the pattern expected to be found. Defaults to `0`
    ///   - distance: 决定匹配必须离模糊的`location`（上面指定的）有多远。一个与模糊位置相差`distance`字符的完全匹配将被视为完全不匹配。`0`的距离要求匹配在指定的确切`位置`，`1000`的距离要求完全匹配在模糊位置的`800`个字符之内，使用0.8的阈值来寻找。默认为`100`。
    ///   Determines how close the match must be to the fuzzy `location` (specified above). An exact letter match which is `distance` characters away from the fuzzy location would score as a complete mismatch. A distance of `0` requires the match be at the exact `location` specified, a `distance` of `1000` would require a perfect match to be within `800` characters of the fuzzy location to be found using a 0.8 threshold. Defaults to `100`
    ///   - threshold: 匹配算法在什么时候会放弃。0.0的阈值要求完全匹配（包括字母和位置），1.0的阈值可以匹配任何东西。默认值为`0.6`。At what point does the match algorithm give up. A threshold of `0.0` requires a perfect match (of both letters and location), a threshold of `1.0` would match anything. Defaults to `0.6`
    ///   - maxPatternLength: 最大的有效模式长度。模式越长，搜索操作就越密集。如果模式超过了`maxPatternLength`，`search`操作将返回`nil`。为什么这很重要？The maximum valid pattern length. The longer the pattern, the more intensive the search operation will be. If the pattern exceeds the `maxPatternLength`, the `search` operation will return `nil`. Why is this important? [Read this](https://en.wikipedia.org/wiki/Word_(computer_architecture)#Word_size_choice). Defaults to `32`
    ///   - isCaseSensitive: 表示比较是否应该区分大小写。Indicates whether comparisons should be case sensitive. Defaults to `false`
    ///   - tokenize: 当为真时，搜索算法将搜索单个单词**和**完整的字符串，计算最终得分作为两者的函数。注意，当 "tokenize "为 "true "时，"threshold"、"distance "和 "location "对单个标记来说是不重要的。When true, the search algorithm will search individual words **and** the full string, computing the final score as a function of both. Note that when `tokenize` is `true`, the `threshold`, `distance`, and `location` are inconsequential for individual tokens.
...
extension Fuse {
        ...
        stride(from: 0, to: count, by: chunkSize).forEach { offset in
            ...
            self.searchQueue.async {
                ...
                for (index, item) in chunk.enumerated() {
                    ...
                    if scores.count == 0 { continue }
                    chunkResult.append((index: offset + index, score: totalScore / Double(scores.count), results: propertyResults
                    ))
                }
    ...
    public func asyncSearch(_ text: String, in aList: [Fuseable], chunkSize: Int = 100) async -> [FusableSearchResult] {

        let pattern = self.createPattern(from: text)
        let count = aList.count
        return await withTaskGroup(of: [FusableSearchResult].self, body: { group in
